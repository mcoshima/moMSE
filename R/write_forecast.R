...
MO_writeforecast <- function (mylist, dir = NULL, file = "forecast.ss", writeAll = FALSE,
                              overwrite = FALSE, verbose = TRUE)
{
  if (verbose)
    cat("running SS_writeforecast\n")
  ...
  printdf <- function(dataframe) {
    ...
    n <- dim(dataframe)[1]*dim(dataframe)[2]
    print.data.frame(dataframe, row.names = FALSE, strip.white = TRUE, max = n)
  }
  ...
  writeLines(paste("#C should work with SS version:",
                   SSversion))
  ...
  if (mylist$Forecast > 0 | writeAll) {
    ...
    if (SSversion == 3.24) {
      if (mylist$fleet_relative_F == 2) {
        ...
        writeLines(paste(mylist$vals_fleet_relative_f,
                         collapse = " "))
      }
      writeLines("# max totalcatch by fleet (-1 to have no max)")
      writeLines(paste(mylist$max_totalcatch_by_fleet,
                       collapse = " "))
      ...
      writeLines(paste(mylist$fleet_assignment_to_allocation_group,
                       collapse = " "))
      if (any(mylist$fleet_assignment_to_allocation_group !=
              0)) {
        writeLines(paste("# allocation fraction for each of:",
                         mylist$N_allocation_groups, " allocation groups"))
    ...
    if (SSversion == "3.30" | SSversion == 3.3) {
      ...
      writeLines("-9999 -1")
      wl("InputBasis")
      if (!is.null(mylist$ForeCatch) && nrow(mylist$ForeCatch >
                                             0)) {
        printdf(mylist$ForeCatch)
      }
